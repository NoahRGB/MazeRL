...
class QLearnerStepper:
  ...
  def iteration_step(self):
      if not self.current_iteration_config["done"]:
          action = self.run_policy(self.current_iteration_config["state"])
          next_state, reward, done = self.environment.step(action, self.current_iteration_config["state"])
          self.current_iteration_config["done"] = done
          self.apply_q_update(self.current_iteration_config["state"], action, reward, next_state, self.current_iteration_config["done"], self.discount_factor)
          self.current_iteration_config["state"] = next_state
          self.current_iteration_config["total_reward"] += reward * (self.discount_factor**self.current_iteration_config["time_step"])
          self.current_iteration_config["time_step"] += 1
          if self.current_iteration_config["time_step"] > 50000:
              raise Exception("Should have solved it by now - something is wrong with the code")

      self.reward_history.append(self.current_iteration_config["total_reward"])
      self.trajectory_length_history.append(self.current_iteration_config["time_step"])
      return self.current_iteration_config["state"], self.current_iteration_config["done"]
