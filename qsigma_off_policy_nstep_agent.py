    ...
    def iteration_step(self):
        ...
        if time_to_update >= 0:
            ...
            for t in range(self.time_step, time_to_update, -1):
                if False: #t == self.termination_time:
                    target = self.rewards[self.termination_time+1]
                else:
                    expected_value = 0
                    ...
                    target = (
                            self.rewards[t+1] +
                            self.discount_factor * (self.sigmas[t]*self.isrs[t] + (1-self.sigmas[t])*chosen_action_prob) *
                            (target - self.qtable[chosen_state_y, chosen_state_x, chosen_action]) +
